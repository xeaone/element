...
const isRadioChecked = function (element) {
    if (element.nodeName === 'INPUT' && element.type === 'radio') {
        const radios = element.ownerDocument.querySelectorAll(`input[name="${element.name}"]`);
...
export const attributeValue = function (element, binder, source, target) {
    if (source === target) {
        return;
    }
    if (!binder.name) {
    ...
    if (isValue(binder.name)) {
        ...
        if (typeof binder.value === 'function') {
            element.removeEventListener(sliceOn(binder.name), binder.value, source?.[1] ?? true);
        }
