...
const eachRender = async function (binder) {
    console.log(binder);
    const parent = binder.value.replace(/\[\s*(?:"|`|').*?(?:'|`|"|)\s*,\s*(.*?)\s*\]/, '$1');
    const [ child, items ] = await binder.compute();

    binder.targetLength = items?.length ?? 0;
    binder.currentLength = binder.currentLength ?? 0;
    binder.templateString = binder.templateString ?? binder.node.innerHTML;
    binder.templateLength = binder.templateLength ?? binder.node.children.length;
    binder.pattern = binder.pattern ?? new RegExp(`(bind=".*?)${child}(.*?")`, 'g');
    binder.templateElement = binder.templateElement ?? document.createElement('template');

    if (!binder.setup) binder.node.innerHTML = '';
    binder.setup = binder.setup ?? true;

    if (binder.currentLength > binder.targetLength) {
        while (binder.currentLength > binder.targetLength) {
            let count = binder.templateLength;

            while (count--) {
                binder.node.removeChild(binder.owner.lastChild);
            }
        ...
        while (binder.currentLength < binder.targetLength) {
            content += binder.templateString.replace(binder.pattern, `$1${parent}[${binder.currentLength}]$2`);
            binder.currentLength++;
        }

        binder.templateElement.innerHTML = content;
        binder.node.appendChild(binder.templateElement.content);
    }
