...
class XRouter extends HTMLElement {
    ...
    constructor(option) {
        ...
        this.#clickInstance = function (event) { instance.#click(event, this); };
    ...
    async #go(path, options) {
        ...
        if (this.#names.has(location.pathname)) {
            element = this.#cache ?
                ...
                document.createElement(this.#names.get(location.pathname));
        }
        else {
            ...
            if (this.#cache)
                this.#elements.set(location.pathname, element);
        }
        if (this.#onBefore)
            await this.#onBefore(location, element);
        globalThis.history.replaceState({
            href: window.location.href,
            top: window.document.body.scrollTop || 0
        }, '', window.location.href);
        if (mode === 'push')
            globalThis.history.pushState({ top: 0, href: location.href }, '', location.href);
        if (mode === 'replace')
            globalThis.history.replaceState({ top: 0, href: location.href }, '', location.href);
    ...
    async #click(event, element) {
        ...
        if (element.hasAttribute('download') ||
            ...
            return;
        if (this.#paths.length && !this.#paths.includes(element.pathname))
            return;
    ...
    async connectedCallback() {
        ...
        this.#observer = new MutationObserver(mutations => mutations.forEach(mutation => {
            ...
            for (const node of removed) {
                if (node.nodeName === 'A')
                    node.removeEventListener('click', this.#clickInstance, true);
                if (node.hasChildNodes() || node?.shadowRoot?.hasChildNodes()) {
